...
function user_job_setup()
	...
	state.Weapons:options('None','Heishi','Daggers','LowBuff','MagicWeapons','ProcDagger','ProcSword','ProcGreatSword','ProcScythe','ProcPolearm','ProcGreatKatana','ProcKatana','ProcClub','ProcStaff')
	...
	send_command('bind @f gs c cycle stance')
	send_command('bind !t gs c set WeaponskillMode Proc;gs c set CastingMode Proc;gs c update')
	send_command('bind ^t gs c set WeaponskillMode Normal;gs c set CastingMode Normal;gs c update')
	send_command('bind @q gs c set weapons Heishi')
...
function init_gear_sets()
    ...
    sets.precast.WS = {
        ...
        body="Adhemar Jacket +1",hands={ name="Herculean Gloves", augments={'Weapon skill damage +3%','DEX+4','Mag. Acc.+9 "Mag.Atk.Bns."+9',}},ring1="Ilabrat Ring",ring2="Regal Ring",
        back="Sacro Mantle",waist="Fotia Belt",legs={ name="Herculean Trousers", augments={'"Rapid Shot"+3','Attack+1','Weapon skill damage +6%','Accuracy+2 Attack+2','Mag. Acc.+10 "Mag.Atk.Bns."+10',}},feet=gear.herculean_wsd_feet
		}
	--	Fotia Gorget
    ...
    sets.precast.WS['Aeolian Edge'] = {ammo="Dosis Tathlum",
        head="Nyame Helm",neck="Deviant Necklace",ear1="Friomisi Earring",ear2="Hattori Earring",
        body="Nyame Mail",hands="Nyame Gauntlets",ring1="Prolix Ring",ring2="Metamor. Ring +1",
        back="Toro Cape",waist="Eschan Stone",legs="Nyame Flanchard",feet="Nyame Sollerets"}
    sets.precast.WS['Blade: Ei'] = {ammo="Dosis Tathlum",
        head="Nyame Helm",neck="Deviant Necklace",ear1="Friomisi Earring",ear2="Hattori Earring",
        body="Nyame Mail",hands="Nyame Gauntlets",ring1="Prolix Ring",ring2="Metamor. Ring +1",
        back="Toro Cape",waist="Eschan Stone",legs="Nyame Flanchard",feet="Nyame Sollerets"}		
    ...
    sets.idle = {
        ...
        body="Malignance Tabard",hands="Malignance Gloves",left_ring="Warp Ring",right_ring="Shneddick Ring",
...
--		ring1="Defending Ring",ring2="Dark Ring",
    ...
    sets.engaged = {
	    ammo="Happo Shuriken",
		head={ name="Adhemar Bonnet", augments={'DEX+9','AGI+9','Accuracy+13',}},
	    body={ name="Adhemar Jacket +1", augments={'DEX+12','AGI+12','Accuracy+20',}},
	    hands={ name="Adhemar Wrist. +1", augments={'DEX+12','AGI+12','Accuracy+19',}},
		legs="Samnuha Tights",
		...
		left_ring="Rajas Ring",ring2="Epona's Ring",
...
--        back=gear.da_jse_back,waist="Windbuffet Belt +1",legs="Samnuha Tights",feet=gear.herculean_ta_feet
	...
	sets.weapons.Daggers = {main="Levante Dagger",sub="Uzura +2"}
	sets.weapons.LowBuff = {main="Yagyu Shortblade",sub="Blurred Knife +1"}
	...
	sets.weapons.ProcPolearm = {main="Sha Wujing's Lance",sub=empty}
	sets.weapons.ProcGreatKatana = {main="Zanmato",sub=empty}
	sets.weapons.ProcKatana = {main="Yagyu Shortblade",sub=empty}
...
function proc_weapons()
	if state.Weapons:contains('sets.weapons.Proc') 
	then
		disable(ammo)
	else
		enable(ammo)
...
function user_job_lockstyle()
		...
		elseif res.items[item_name_to_id(player.equipment.main)].skill == 9 then --Sword in main hand.
			if res.items[item_name_to_id(player.equipment.sub)].skill == 9 then --Katana/Katana.
				windower.chat.input('/lockstyleset 016')
			elseif res.items[item_name_to_id(player.equipment.sub)].skill == 2 then --Katana/Dagger.
				windower.chat.input('/lockstyleset 015')
			elseif res.items[item_name_to_id(player.equipment.sub)].skill == 11 then --Katana/Club.
				windower.chat.input('/lockstyleset 018')
			else
				windower.chat.input('/lockstyleset 014') --Catchall
			end
		elseif res.items[item_name_to_id(player.equipment.main)].skill == 3 then --Sword in main hand.
			if res.items[item_name_to_id(player.equipment.sub)].skill == 9 then --Sword/Katana.
